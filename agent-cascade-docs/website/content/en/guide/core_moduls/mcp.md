...
MCP (Model Context Protocol) is a standardized protocol that enables large language models (LLMs) to interact with external tools and services in a structured way. In the AgentCascade framework, MCP is deeply integrated to empower intelligent agents with capabilities such as file system access, memory management, database queries, and more.

This document provides a comprehensive guide on how to configure and use MCP within AgentCascade.

---

## 1. Prerequisites

### 1.1 Install AgentCascade with MCP Support

Install the stable version from PyPI:

```bash
pip install -U "agent-cascade[mcp]"
```
...
2. **Service Lifecycle**:
   - AgentCascade automatically starts configured MCP services when the agent is initialized.
...
3. **Debugging Tips**:
   ...
   - Test with official examples, such as [`assistant_mcp_sqlite_bot.py`](https://github.com/QwenLM/AgentCascade/blob/main/examples/assistant_mcp_sqlite_bot.py).
...
- [AgentCascade MCP Example: SQLite Bot](https://github.com/QwenLM/AgentCascade/blob/main/examples/assistant_mcp_sqlite_bot.py)
- [MCP Cookbooks (Added May 2025)](https://github.com/QwenLM/AgentCascade/tree/main/examples)

---

By properly configuring MCP in AgentCascade, you can significantly extend your agent’s ability to interact with the real world—enabling robust, context-aware applications powered by Qwen’s reasoning and tool-use capabilities.
