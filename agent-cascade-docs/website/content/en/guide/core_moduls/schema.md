# AgentCascade Schema Documentation


## Overview

The `agent-cascade` schema provides a structured, type-safe messaging system to support advanced capabilities such as multimodal conversations, function calling, and reasoning chains.
...
- The Agent Cascade receives and returns a list of messages, and the `reasoning_content`, `content`, and `function_call` in a response will be stored in separate messages.
