...
Currently, AgentCascade provides access interfaces to Qwen's DashScope API and OpenAI API, and Qwen-VL's DashScope API. Both already support streaming Function Calling.
...
LLM classes uniformly use the `llm.chat(...)` interface to generate responses, supporting input of message lists, functions, and other parameters. For a detailed parameter list, see [BaseChatModel](../../../../../../agent_cascade/llm/base.py).
...
## 2. LLM Development
AgentCascade provides a mechanism for registering LLMs. In the [LLM Base Class](../../../../../../agent_cascade/llm/base.py), a uniform `llm.chat(...)` interface is implemented.
...
If the newly registered LLM does not support function calls, it can inherit from the [BaseFnCallModel](../../../../../../agent_cascade/llm/function_calling.py) class implemented in AgentCascade.
