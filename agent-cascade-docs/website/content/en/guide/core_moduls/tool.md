...
```py
from agent_cascade.tools import ImageGen
...
Note that for the convenience of inputting tool results into LLMs, the Agent’s `_call_tool(...)` interface converts all returned results from the tools into string type. For more details, see the [Agent class](../../../../../../agent_cascade/agent.py).

## 2. Tool Development

AgentCascade provides a mechanism for registering tools. For example, to register your own image generation tool:
