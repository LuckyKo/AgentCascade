# AgentCascade Features

AgentCascade is a powerful and flexible framework for building intelligent LLM-powered applications. Key features include:
...
- **Built-in Tool Call Parser**
  Adapted to Qwen's tool call template, agent-cascade can still use the model's tool calling capability normally when the model service does not support tool call parser. It also supports the use of the tool call parser that comes with the model service.
