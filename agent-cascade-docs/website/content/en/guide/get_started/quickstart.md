...
- AgentCascade supports reading the tool call response of the model, which requires enabling the tool call parser parameter when deploying the model.
...
AgentCascade's built-in tool call parser

- Because some model deployment methods do not provide native tool call capabilities, or tool call support is incomplete, AgentCascade's built-in tool call parsing can be used.
AgentCascade provides `hermes` parsing format by default, supporting tool call parsers for QwQ and Qwen3 series model. Currently, this is the default mode.
- For the QwQ and Qwen3 series model, it is recommended to use the built-in tool call parser of AgentCascade, i.e. **do not** add the `--enable-auto-tool-choice` and `--tool-call-parser hermes` parameters in vLLM/SGLang server.
