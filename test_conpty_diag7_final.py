"""
ConPTY Diagnostic v7 — FINAL: uses raw CreateProcessW with STARTUPINFOEXW.

This proves the fix: when you properly pass STARTUPINFOEXW to CreateProcessW,
the PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE attribute is applied and output IS captured.

The bug in async_shell.py: it creates STARTUPINFOEXW but doesn't pass it to
subprocess.Popen, so the ConPTY attachment never happens.
"""
...
def create_inheritable_pipe():
    ...
    # NOTE: Try WITHOUT making handles inheritable — ConPTY attachment doesn't need inherited pipes
...
def main():
    print("=" * 60, flush=True)
    print("ConPTY Diagnostic v7 — FINAL (raw CreateProcessW)", flush=True)
    print("=" * 60, flush=True)
    ...
    si = STARTUPINFOEXW()
    si.StartupInfo.cb = ctypes.sizeof(STARTUPINFOEXW)  # MUST be sizeof(STARTUPINFOEXW)
    ...
    # ── Spawn using RAW CreateProcessW with STARTUPINFOEXW ────────────
    command = ctypes.c_wchar_p('powershell.exe -NoProfile -Command "for ($i=1; $i -le 5; $i++) { Write-Host \"Line $i\"; Start-Sleep -Milliseconds 500 }"')
    pi = PROCESS_INFORMATION()

    # KEY: bInheritHandles=False because ConPTY attachment is via attribute list, not inherited pipes
    creation_flags = 0x00000200 | 0x00080000  # CREATE_NEW_PROCESS_GROUP | EXTENDED_STARTUPINFO_PRESENT

    print(f"\n[SETUP] Spawning via raw CreateProcessW with STARTUPINFOEXW...", flush=True)
    ...
    success = kernel32.CreateProcessW(
        ...
        False,                         # bInheritHandles — FALSE! ConPTY via attribute list
        creation_flags,                # EXTENDED_STARTUPINFO_PRESENT
        None,                          # lpEnvironment
        None,                          # lpCurrentDirectory
        ctypes.byref(si),              # lpStartupInfo — STARTUPINFOEXW with ConPTY attribute!
    ...
    expected_keywords = {"Line 1", "Line 2", "Line 3", "Line 4", "Line 5"}
    ...
    if found_keywords == expected_keywords:
        ...
        print("Root cause confirmed: async_shell.py must pass STARTUPINFOEXW to CreateProcess.")
        return 0
    elif captured_data:
