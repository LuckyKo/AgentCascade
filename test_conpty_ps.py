"""
ConPTY test with PowerShell that explicitly flushes output and waits.
Uses exact diag7_final ctypes setup.
"""
...
def main():
    print("=" * 60, flush=True)
    print("ConPTY Test with PS (interactive cmd.exe)", flush=True)
    print("=" * 60, flush=True)
    ...
    # ── Spawn interactive cmd.exe (like diag10) ───────────────────────
    ...
    print(f"\n[SETUP] Spawning interactive cmd.exe via CreateProcessW...", flush=True)
