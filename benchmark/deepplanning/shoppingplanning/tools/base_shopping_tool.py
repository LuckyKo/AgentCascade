...
Framework-agnostic, designed to be compatible with the agent-cascade BaseTool interface.
Intended to be usable both as a standalone open-source tool and easily integrated into the agent-cascade framework.
"""
...
class BaseShoppingTool(ABC):
    ...
    2. Implements key agent-cascade BaseTool interface for easy integration
    ...
    - Integration into agent-cascade: via an adapter class
    ...
    def function(self) -> Dict:
        """
        Retrieve function definition (compatible with agent-cascade BaseTool interface).
