"""Test following Microsoft's exact ConPTY sample pattern."""
...
kernel32.CreateProcessW.argtypes = [
    ...
    ctypes.POINTER(STARTUPINFOW),  # MS docs: pass pointer to StartupInfo member!
...
def main():
    ...
    if not kernel32.InitializeProcThreadAttributeList(attr_buf, 1, 0, ctypes.byref(size_attr)):
        raise RuntimeError(f"InitAttr failed: {ctypes.WinError()}")
    ...
    # Spawn cmd.exe — interactive (no /c) so it stays alive
    cmdline = "cmd.exe"
    ...
    success = kernel32.CreateProcessW(
        ...
        None, None, False,
        EXTENDED_STARTUPINFO_PRESENT,  # MS sample uses ONLY this flag
        None, None,
        ctypes.byref(si.StartupInfo),  # KEY: pass pointer to StartupInfo member!
        ctypes.byref(pi),
    )
    ...
    # Send command via input pipe (UTF-16LE as ConPTY expects)
    ...
    print(f"WriteFile to ConPTY: ok={ok}, written={written.value}")
