...
class OperationManager:
    ...
    def cleanup_backups(self, agent_name: Optional[str] = None):
        ...
        try:
            ...
            if agent_name:
                # Sanitize agent name for use as a directory name (agent names may contain special chars)
                ...
                # Clean up old timestamped archives (runs even when no .bak files exist)
                ...
                # Remove .bak files after successful archiving (graceful on individual failures)
                if bak_files:
                    for bak_file in bak_files:
                        try:
                            bak_file.unlink()
                ...
                # Clean up old timestamped archives at base level (runs even when no .bak files exist)
    ...
    # ── Static helpers for list_directory (fixes #4, #5) ───────────────
    ...
    def _list_flat(
        ...
        # Apply max_entries cap: dirs first, then fill with files
        overflow_count = 0
    ...
    # ── Extracted recursive listing (fix #3) ───────────────────────────
    ...
    def _list_recursive(
        ...
        entries_by_dir: dict = OrderedDict()  # dir_path -> sorted entry list
        ...
        for dirpath, subdirs, filenames in os.walk(str(resolved), topdown=True):
            ...
            # Fix #1: reuse abs_path instead of calling .resolve() again
        ...
        # Fix #8: compute summary stats from collected data (not during formatting)
        ...
        output = ""
        # Fix #7: replace decorative headers with plain brackets
    ...
    def list_directory(
        ...
        try:
            ...
            # Fix #2: normalize all negative max_depth to -1 (unlimited)
            if max_depth < 0:
                max_depth = -1
    ...
    def edit_file(self, path: str, agent_name: str,
        ...
        if match_mode == 'exact':
            ...
            def _normalize_line_python(line: str) -> str:
                ...
                """
                result = line
                # Remove string literals (double-quoted, handles basic cases)
                result = re.sub(r'"[^"]*"', '', result)
                # Remove string literals (single-quoted)  
                result = re.sub(r"'[^']*'", '', result)
                # Remove numeric literals: floats FIRST, then integers (order matters!)
                result = re.sub(r'\[(\d+\.\d+)\]', '[]', result)  # floats in brackets first to prevent empty artifacts
                result = re.sub(r'\b\d+\.\d+\b', '', result)  # floats like 3.14
                result = re.sub(r'\b\d+\.?\d*[eE][+-]?\d+\b', '', result)  # scientific notation (before integers!)
