...
def _is_stale_prefix_of_serialized(
    ...
    TOOL-CALL EXEMPTION: this guard applies ONLY to text/reasoning partials. A tool-call message
    (``stream_func_call is not None``) is NEVER treated as a stale prefix — its content is '' and
    it is identified by function_call, so an empty-content "prefix" match against the last
    serialized assistant would wrongly drop every parallel tool bubble after the first (the
    reported UI freeze). Tool-call dedup is handled by the id-aware fingerprint instead.
    """
    sc = stream_content or ''
    sr = stream_reasoning or ''

    # This guard exists ONLY for TEXT/REASONING partials: it catches a stale text prefix that is
    # already contained in the last committed assistant message. It must NEVER apply to tool-call
    # messages — those have content=='' and are identified by function_call, so an empty-content
