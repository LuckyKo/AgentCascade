...
def _parse_cooldown_env():
    try:
        val = int(os.environ.get("AGENT_CASCADE_ENDPOINT_COOLDOWN", "60"))
        return max(0, val)  # Clamp to 0 if negative
    except (ValueError, TypeError):
        return 60

ENDPOINT_COOLDOWN_SECONDS = _parse_cooldown_env()  # Seconds to skip a failed endpoint

# Cleanup threshold: remove failure records older than this (hours)
ENDPOINT_FAILURE_CLEANUP_HOURS = 24
    ...
    def get_endpoint_chain(
        ...
        with self._lock:
            ...
            if ENDPOINT_COOLDOWN_SECONDS > 0 and configs:
                ...
                if skipped_count > 0:
                    configs = filtered_configs if filtered_configs else []
                    logger.info(
                        f"[APIRouter] Endpoint cooldown: skipped {skipped_count} endpoint(s), "
                        f"{len(configs)} available for '{normalized_agent_type}'"
                    )
        ...
        for cfg_idx, llm_cfg in enumerate(chain):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    # model is unlikely to help — a stuck character repetition or token
                    # overflow means degraded model state and warrants endpoint switch.
                    # This bridges Layer 2 (endpoint iteration) with detection from
                    # execution_engine.py.
                    if isinstance(e, (CharacterRunDetected, MaxTokenExceeded)):
                        logger.warning(
                            f"[APIRouter] Character run/token limit detected for '{endpoint_name}' @ {endpoint_base}. "
                            f"Advancing to next endpoint."
                        )
                        all_errors.append(f"Character run/token limit detected for '{endpoint_name}' @ {endpoint_base}: {err_msg}")
