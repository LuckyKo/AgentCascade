...
class TelemetryCollector:
    ...
    def __init__(self, log_dir: str = "workspace/telemetry"):
        ...
        # Session-level tracking — use _now_iso() for consistent UTC timestamps (Issue #9)
        ...
        # In-memory event buffer — deque with maxlen gives O(1) rotation (Issue #7)
        ...
        self._session_stats = {
            ...
            "write_failures": 0,  # Issue #2: track how many writes failed
        ...
        # Keep file handle open to avoid per-event open/close overhead (Issue #6)
        self._log_file = None
        try:
    ...
    # ── Helper: per-config stats initialization (Issue #4) ───────────────
    ...
    def record_turn_start(
        ...
        with _telemetry_lock:  # Issue #1: protect all shared state
            # Issue #5: Warn if an active turn already exists with a different fingerprint
            ...
            # Initialize per-config stats on first turn (Issue #4: extracted to helper)
        ...
        with _telemetry_lock:  # Issue #1: protect all shared state
            ...
            if not turn:
                _logger.warning(
                    ...
                    instance_name,
                )  # Issue #8: warn on mismatched turn start/end
                return
            ...
            # Update per-config stats — use helper to avoid duplication (Issue #4)
            if fp:
    ...
    def record_llm_call_start(self, instance_name: str, input_tokens_est: int = 0, model: str = ""):
        ...
        with _telemetry_lock:  # Issue #1: protect shared state
    ...
    def get_session_summary(self) -> Dict:
        ...
        with _telemetry_lock:  # Thread safety for read access
        ...
        return {
            ...
            "write_failures": stats.get("write_failures", 0),  # Issue #2: expose in summary
    ...
    def record_session_end(self):
        ...
        Robust against partial write failures: logs summary at INFO level as fallback (Issue #3).
        """
        with _telemetry_lock:  # Thread safety for session_ended guard + stats reads (Issue 2)
        ...
        try:
            ...
            # Issue #3: fallback — log the full summary at INFO so it appears in console logs
            _logger.info(
    ...
    def _write_critical_event(self, event: Dict):
        ...
        Thread-safe via module-level lock (Issue #1). Uses cached file handle (Issue #6).
        """
        self.events.append(event)

        line = json.dumps(event, ensure_ascii=False, default=str) + "\n"
        with _telemetry_lock:  # Issue #1: thread safety
    ...
    def _write_event(self, event: Dict):
        ...
        Thread-safe via module-level lock (Issue #1). Uses cached file handle (Issue #6).
        Failures are logged at WARNING level with a failure counter (Issue #2).
        ...
        try:
            with _telemetry_lock:  # Issue #1: thread safety
                if self._log_file is not None and not self._log_file.closed:
                    self._log_file.write(line)
                    self._log_file.flush()  # Reviewer fix: flush to avoid data loss on crash
                else:
                    # Fallback: reopen file handle so subsequent writes don't pay open/close cost
                    fh = open(self.log_path, "a", encoding="utf-8")
                    fh.write(line)
            ...
            _logger.warning(  # Issue #2: upgraded from debug to warning
                "Failed to write telemetry event [%s]: %s", event.get("type", "unknown"), e,
            )
            self._session_stats["write_failures"] += 1

    def close(self):
        """Flush and close the log file handle. Call during shutdown cleanup (Issue #6)."""
