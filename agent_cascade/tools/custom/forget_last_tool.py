...
from agent_cascade.settings import DEFAULT_FORGET_LAST_MIN_CHAR_LIMIT, DEFAULT_FORGET_LAST_TRUNCATE_MAX_CHARS
...
class ForgetLast(BaseTool):
    ...
    parameters = {
        ...
        'properties': {
            'count': {
                'type': 'integer',
                'description': ('Number of recent tool call responses to truncate. '
            ...
            'justification': {
                'type':
                    'string',
                'description': ('Optional reason for truncation. Appended to the truncation marker '
    ...
    def call(self, params: Union[str, dict], **kwargs) -> str:
        ...
        agent_name = (
            kwargs.get('agent_instance_name') or (getattr(agent_obj, 'instance_name', None) if agent_obj else None) or
            self.agent_name or
        ...
        logger.info(f"ForgetLast[{agent_name}]: truncated {truncated_count}/{n} responses, "
