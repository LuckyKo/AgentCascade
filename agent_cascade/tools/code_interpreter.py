...
# Optional ZMQ import for explicit error handling on kernel shutdown
try:
    import zmq
except ImportError:
    zmq = None
...
from agent_cascade.utils.code_path_resolver import build_path_resolution_notice, resolve_code_paths, set_active_mappings
from agent_cascade.utils.utils import (append_signal_handler, extract_code, has_chinese_chars, json_loads,
                                       print_traceback)
...
# Per-session tracking: session_name → list of kernel_ids owned by that session.
# Enables explicit cleanup when agents are dismissed instead of relying on GC.
_AGENT_KERNELS: Dict[str, List[str]] = {}
...
def _kernel_watchdog():
    ...
    while not _WATCHDOG_TERMINATE.is_set():
        ...
        for kernel_id, wd_timeout in stale_kernels:
            logger.warning(f"Code interpreter watchdog: Kernel {kernel_id} inactive for "
            ...
            with _KERNEL_LOCK:
                ...
                # Remove from agent kernel tracking
            ...
            if container_id_to_kill is not None:
                try:
                    subprocess.run(['docker', 'stop', container_id_to_kill],
                                   timeout=10,
                                   capture_output=True,
                                   encoding='utf-8',
                                   errors='replace')
                    subprocess.run(['docker', 'rm', container_id_to_kill],
                                   timeout=10,
                                   capture_output=True,
                                   encoding='utf-8',
                                   errors='replace')
...
def cleanup_kernels_for_session(session_name: str, force_timeout: float = 5.0) -> int:
    """Explicitly clean up all kernels owned by a session (agent).

    This is the primary mechanism for preventing ZMQ socket leaks when agents
    are dismissed. It shuts down kernel clients and removes containers for all
    kernels tracked under the given session name.

    Uses cooperative shutdown first, then force-kills containers if they don't
    respond within force_timeout seconds.

    Args:
        session_name: The session/agent name whose kernels should be cleaned up.
        force_timeout: Max seconds to wait for docker stop before forcing removal.

    Returns:
        Number of kernels cleaned up (0 if none found).
    """
    ...
    with _KERNEL_LOCK:
        if session_name not in _AGENT_KERNELS:
            return 0
        ...
        for kid in kernel_ids:
            ...
            if kid in _STALE_CONTAINERS:
                stale_cids.append(_STALE_CONTAINERS.pop(kid, None))
            if kid in _KERNEL_ACTIVITY:
                kernel_work_dirs[kid] = _KERNEL_ACTIVITY[kid].get('work_dir', '.')
                del _KERNEL_ACTIVITY[kid]
    ...
    for kid, cid in container_pairs:
        if cid is None:
            continue
        try:
            subprocess.run(['docker', 'stop', '-t', str(int(force_timeout)), cid],
                           timeout=force_timeout + 5,
                           capture_output=True,
                           encoding='utf-8',
                           errors='replace')
    ...
    for info in stale_cids:
        ...
        if cid:
            try:
                ...
                logger.debug(f"Failed to remove stale container {cid}: {e}")

    # Clean up temporary files: connection files and launch scripts
    for kid, work_dir in kernel_work_dirs.items():
        # Remove connection files
        for suffix in ['_host.json', '_container.json']:
            conn_file = os.path.join(work_dir, f'kernel_connection_file_{kid}{suffix}')
            try:
                if os.path.exists(conn_file):
                    os.remove(conn_file)
            except OSError as e:
        ...
        launch_script = os.path.join(work_dir, f'launch_kernel_{kid}.py')
        try:
            if os.path.exists(launch_script):
                os.remove(launch_script)
        except OSError as e:
...
def _cleanup_single_kernel(kernel_id: str, work_dir: str, force_timeout: float = 5.0):
    ...
    if stale_info is not None:
        ...
        if stale_cid:
            try:
                subprocess.run(['docker', 'rm', '-f', stale_cid],
                               timeout=10,
                               capture_output=True,
                               encoding='utf-8',
                               errors='replace')
...
class CodeInterpreter(BaseToolWithFileAccess):
    ...
    parameters = {
        ...
        'properties': {
            ...
            'fix_paths': {
                'description':
                    'Auto-translate Windows host paths (e.g. N:\\work\\...) to Docker container paths (/workspace/...). Set to false to disable.',
                'type':
                    'boolean',
                'default':
                    True,
    ...
    def call(self, params: Union[str, dict], files: List[str] = None, timeout: Optional[int] = None, **kwargs) -> str:
        ...
        try:
            ...
            code = validated_params.get('code', '')
               ...
               ):  # _verify_json_format_args can raise ValueError or jsonschema.ValidationError on failure
        ...
        if kc is None:
            ...
            with _KERNEL_READY:
                ...
                if kernel_id in _STALE_CONTAINERS:
                    del _STALE_CONTAINERS[kernel_id]
                # Track which session owns this kernel for explicit cleanup on dismissal
                if session_name not in _AGENT_KERNELS:
                    _AGENT_KERNELS[session_name] = []
                if kernel_id not in _AGENT_KERNELS[session_name]:
                    _AGENT_KERNELS[session_name].append(kernel_id)
                _KERNEL_READY.notify_all()
        ...
        try:
            ...
            if not interrupted:
                ...
                if container_id:
                    try:
                        subprocess.run(['docker', 'exec', container_id, 'kill', '-INT', '1'],
                                       timeout=5,
                                       capture_output=True,
                                       encoding='utf-8',
                                       errors='replace')
            ...
            if not interrupted:
                ...
                if container_id:
                    try:
                        subprocess.run(['docker', 'kill', '-s', 'KILL', container_id],
                                       timeout=10,
                                       capture_output=True,
                                       encoding='utf-8',
                                       errors='replace')
            ...
            if was_killed:
                logger.warning(f"Code interpreter execution failed because the kernel was killed "
                               f"by the watchdog (inactive for {CONTAINER_WATCHDOG_TIMEOUT}s). "
                               f"Original error: {e}")
                return (f'ERROR: Code interpreter kernel was terminated due to inactivity '
        ...
        if char_limit != -1 and len(result) > char_limit:
            ...
            # Use agent_pool's base_dir for correct relative path resolution if available
    ...
    def _drain_iopub(
        ...
        while True:
            ...
            if mtype == 'status':
                ...
                logger.debug(f"IOPub drain: ignoring unknown msg type '{mtype}' "
                             f"for kernel {kernel_id}")

        # Consolidate stdout/stderr into single blocks (fix output splitting bug)
        parts = []
        if stdout_buf:
            _stdout_joined = ''.join(stdout_buf)
            parts.append(f'\n\nstdout:\n```\n{_stdout_joined}\n```')
        if stderr_buf:
            _stderr_joined = ''.join(stderr_buf)
            parts.append(f'\n\nstderr:\n```\n{_stderr_joined}\n```')
        ...
        if len(partial_output) > drain_max_output_bytes:
            ...
            partial_output = (f'{kept}\n\n[OUTPUT TRUNCATED — exceeded '
                              f'{drain_max_output_bytes // (1024*1024)}MB limit]')

        return interrupted, partial_output

    def close(self) -> int:
        """Explicitly shut down all kernels owned by this CodeInterpreter instance.

        Call this method to ensure ZMQ sockets are properly released instead of
    ...
    def __del__(self):
        ...
        # NOTE: This is a fallback — explicit cleanup via close() or cleanup_kernels_for_session()
    ...
    def _process_extra_folders(self, folders: List[str], allowed_prefixes: set, is_ro: bool) -> List[dict]:
        ...
        for folder_path in folders:
            ...
            if not self._is_path_allowed(resolved, allowed_prefixes):
                logger.warning('Extra %s mount path %s is outside allowed directories, skipping', label, resolved)
                continue
    ...
    def _build_path_mapping(self, kernel_id: str, mounted_rw: List[dict], mounted_ro: List[dict]) -> dict:
        ...
        work_host = _normalize_path_for_docker(os.path.realpath(self.work_dir) if self.work_dir else '')
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Kernel not ready (attempt {attempt + 1}/{max_retries}, "
    ...
    def _start_kernel(self, kernel_id: str):
        ...
        docker_run_cmd.extend([
            '-v',
            f'{_normalize_path_for_docker(os.path.realpath(self.work_dir))}:{self.container_work_dir}',
            '-w',
        ...
        docker_run_cmd.extend([
            ...
            'python',
            f'{self.container_work_dir}/{os.path.basename(launch_kernel_script)}',
    ...
    def _warm_restart_kernel(self, kernel_id: str, container_id: str):
        ...
        exec_cmd = [
            ...
            container_id,
            'python',
            f'{self.container_work_dir}/{os.path.basename(launch_script)}',
            '--IPKernelApp.connection_file',
            f'{self.container_work_dir}/{os.path.basename(container_connection_file)}',
            '--KernelApp.allow_remote_access=False',
            '--matplotlib=inline',
    ...
    def _execute_code(self, kc, code: str, timeout: Optional[int] = None, kernel_id: Optional[str] = None) -> str:
        ...
        except (zmq.ZMQError, queue.Empty):
            pass
        ...
        while True:
            ...
            if was_killed:
                ...
                text = (f'ERROR: The code interpreter kernel was terminated due to '
