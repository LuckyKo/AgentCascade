...
class TextChatAtOAI(BaseFnCallModel):
    ...
    def _chat_stream(
        ...
        try:
            logger.info(f"[TIMING] _chat_stream START model={request_model}")
            response = self._chat_complete_create(model=request_model, messages=messages, stream=True, **generate_cfg)
            logger.info(f"[TIMING] _chat_stream GOT ITERATOR (headers received) model={request_model}")
            if delta_stream:
                ...
                for chunk in response:
                    if _first_chunk:
                        logger.debug(f"[TIMING] _chat_stream FIRST CHUNK model={request_model}")
                        _first_chunk = False
