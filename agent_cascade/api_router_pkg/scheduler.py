...
class EndpointScheduler:
    ...
    def acquire(
        ...
        **kwargs,
        ...
        if concurrency_limit == -1:  # unlimited — no scheduling needed
            logger.debug(
                f"[CALL_AGENT_DEBUG] EndpointScheduler.acquire — api_base={api_base}, concurrency=-1 (unlimited), returning None"
            )
            return None
        ...
        logger.debug(f"[CALL_AGENT_DEBUG] EndpointScheduler.acquire — api_base={api_base}, "
        ...
        try:
            ...
            logger.debug(f"[SLOTPOOL] instance={instance_name} pool={slot_key} "
            ...
            logger.error(f"[EndpointScheduler] Acquire failed for '{instance_name}' on '{api_base}': {e}",
                         exc_info=True)
            raise
    ...
    def get_slot_holders(self, slot_key: str = None) -> Dict[str, List[tuple]]:
        ...
        if slot_key:
            ...
            if pool:
                holders = [
                    (h.instance_name, h.agent_name, h.granted_at, h.acquisition_id) for h in pool._running.values()
                ]
        ...
        for key, pool in self._pools.items():
            ...
            for holder in holders_snapshot:
                ...
                if held_duration > threshold_seconds:
                    ...
                    logger.warning(f"[SLOT_STUCK_DETECTION] Slot on '{key}' held by '{holder.instance_name}' "
        ...
        for key, pool in self._pools.items():
            ...
            for ticket in waiters_snapshot:
                ...
                if age > QUEUE_WAIT_TIMEOUT:
                    ...
                    logger.warning(f"[SLOT_STUCK_DETECTION] Waiter '{ticket.instance_name}' on '{key}' "
