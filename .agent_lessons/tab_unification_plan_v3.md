...
**Version:** 3.0 (Revised based on Final Reviewer Feedback)
...
*   **Action:** Create `config/unified.py` module with exactly these constants:
    ```python
    USE_UNIFIED_ARCHITECTURE = False  # Master toggle - gates all unified behavior
    USE_UNIFIED_STATE = False         # Gates state read/write path
    USE_UNIFIED_LOOP = False          # Gates message loop unification
    ```
*   **Implementation Note:** Access via `from config.unified import USE_UNIFIED_STATE` etc. All default values are `False` (legacy mode).
...
*   **Action:** 
    ...
    *   Implement wrapper function: `def get_session_history(session, instance_name='root', use_unified=None) -> list`.
    *   **Behavior Contract:** Returns `session['history']` if `use_unified` is `False` or `None`; returns `unified_store[instance_name]['messages']` if `True`.
...
*   **Action:** 
    ...
    *   **Concurrency Strategy:** 
        *   Use `threading.Lock` for the unified state store (`sub_agent_state`).
        *   Implement **per-instance locks** (one lock per agent instance) to minimize contention.
        *   **Deadlock Prevention:** Always acquire locks in alphabetical order by instance name.
...
*   **Action:** 
    ...
    *   **Bug Fix - `__USE_PREV_ARG__`:** Move placeholder resolution into `_stream_sub_agent_call()` or a shared utility function called by both streaming and non-streaming paths to fix current inability of streaming tools to use it.
...
**Emergency Kill Switch:** The `USE_UNIFIED_ARCHITECTURE = False` environment variable can be toggled to revert to legacy behavior without code redeployment, **provided all paths in the codebase check this flag**. If any path bypasses the flag, a full rollback is required.
