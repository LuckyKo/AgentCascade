...
**Emergency Kill Switch:** The `AC_USE_UNIFIED_ARCHITECTURE=0` environment variable (which sets `USE_UNIFIED_ARCHITECTURE = False`) can be toggled to revert to legacy behavior without code redeployment, **provided all paths in the codebase check this flag**. If any path bypasses the flag, a full rollback is required.
