...
def heuristic_match(file_content, old_content):
    ...
    for start_idx in candidates:
        ...
        for size in range(max(1, n_old - 2), min(n_file - start_idx + 1, n_old + 3)):
            ...
            ratio = difflib.SequenceMatcher(None, ''.join(old_line_info), ''.join(candidate_norms)).ratio()
            if ratio > best_ratio:
                best_ratio = ratio
                best_info = {'start_list_idx': start_idx, 'end_list_idx': start_idx + size, 'ratio': ratio}
...
def test_no_comment_stripping():
    ...
    old_with_comment = ('def func():\n'
                        '    # Important comment\n'
                        '    x = 1\n')
    old_without_comment = ('def func():\n'
                           '    x = 1\n')
