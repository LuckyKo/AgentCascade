...
def main():
    ...
    check('start_line=5 on 3-line file returns error', 'ERROR' in result and 'beyond the end of file' in result, result)

    # T2: Normal read within bounds
    result = op_mgr.read_file('small.txt', start_line=1, limit=3)
    check('Normal read lines 1-3 returns content', 'line1:' in result and 'line3:' in result and 'ERROR' not in result,
          result)

    # T3: start_line exactly at last line
    result = op_mgr.read_file('small.txt', start_line=3, limit=10)
    check('start_line=3 (last line) returns content', 'line3:' in result and 'ERROR' not in result, result)

    # T4: Empty file
    empty_file = Path(tmpdir) / 'empty.txt'
    empty_file.write_text('', encoding='utf-8')
    result = op_mgr.read_file('empty.txt', start_line=1, limit=10)
    check('Empty file (0 lines), start_line=1 returns error', 'ERROR' in result and 'beyond the end of file' in result,
          result)
    ...
    check('Single-line file read works', 'only line:' in result and 'ERROR' not in result, result)

    # T6: start_line = total_lines + 1 (just beyond)
    result = op_mgr.read_file('small.txt', start_line=4, limit=5)
    check('start_line=4 on 3-line file returns error', 'ERROR' in result and 'beyond the end of file' in result, result)
    ...
    check('Read hits real EOF without [TRUNCATED]', 'line2:' in result and 'line3:' in result and
          '[TRUNCATED]' not in result, result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              agent_name='test_agent',
                              lines='5:10',
                              indent=4,
                              indent_type='space',
                              mode='min')
    check('start_line=5 on 4-line file returns bounds error', 'ERROR' in result and 'exceeds file length' in result,
          result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('end_line=10 on 4-line file returns bounds error', 'ERROR' in result and 'exceeds file length' in result,
          result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('Negative start_line=-3 returns >= 1 error (dead code removed)', 'ERROR' in result and '>= 1' in result,
          result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('Negative end_line=-5 returns >= 1 error', 'ERROR' in result and '>= 1' in result, result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('Normal re_indent returns APPROVED', 'APPROVED' in result, result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check("start==end returns 'Start must be less than end' error", 'ERROR' in result and
          'Start must be less than end' in result, result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('end=total_lines+1 returns bounds error (not silent clamp)', 'ERROR' in result and
          'exceeds file length' in result, result)
    ...
    result = op_mgr.re_indent(path='indent_test.py',
                              ...
                              indent_type='space',
                              mode='min')
    check('Dead code removed: -2:-1 returns >= 1 error (not converted)', 'ERROR' in result and '>= 1' in result, result)

    # T17: Empty file re_indent edge case
    empty_file2 = Path(tmpdir) / 'empty_indent.py'
    empty_file2.write_text('', encoding='utf-8')
    result = op_mgr.re_indent(path='empty_indent.py',
                              agent_name='test_agent',
                              lines='1:5',
                              indent=4,
                              indent_type='space',
                              mode='min')
    check('Empty file (0 lines) returns bounds error for any range', 'ERROR' in result and
          'exceeds file length' in result, result)

    # T18: Valid boundary case still works
    test_file3 = Path(tmpdir) / 'boundary_test.py'
    test_file3.write_text('a\nb\nc\n', encoding='utf-8')  # 3 lines
    result = op_mgr.re_indent(path='boundary_test.py',
                              agent_name='test_agent',
                              lines='1:3',
                              indent=2,
                              indent_type='space',
                              mode='min')
    check('Valid range 1:3 on 3-line file works correctly', 'APPROVED' in result, result)

    # T19: Large start_line returns error immediately (no iteration)
    result = op_mgr.read_file('small.txt', start_line=100, limit=1)
    check('Large start_line=100 returns error immediately', 'ERROR' in result and 'beyond the end of file' in result,
          result)
