...
from agent_cascade.engine.helpers import _build_resources_block, _get_active_functions_from_template
...
class TestGetActiveFunctionsDefensiveChecks:
    ...
    def test_with_instance_override_no_llm(self):
        ...
        inst = _make_mock_instance(generate_cfg_override={'disabled_tools': {'TestAgent': ['bad_tool']}})
        func_good = MagicMock()
...
class TestBuildResourcesBlockFallback:
    ...
    def test_override_exists_but_lacks_disabled_tools_key(self):
        ...
        inst = _make_mock_instance(generate_cfg_override={'max_input_tokens': 80000})
    ...
    def test_override_has_disabled_tools_takes_precedence(self):
        ...
        inst = _make_mock_instance(generate_cfg_override={'disabled_tools': ['override_tool']})
...
class TestDisabledToolsMergeOnSettingsPropagation:
    ...
    def test_merge_disabled_tools_via_execution_engine(self):
        ...
        tmpl_llm = MagicMock()
        tmpl_llm.generate_cfg = {'disabled_tools': {'TestChild': ['ui_disabled_tool']}}
        ...
        parent_llm.generate_cfg = {
            'disabled_tools': {
                'TestParent': ['parent_disabled_tool']
            },
        ...
        with mock_pool._execution._state_lock:
            ...
            if caller_disabled_tools:
                cfg = dict(inst._generate_cfg_override or
                           {}) if inst._generate_cfg_override else (target_template.llm.generate_cfg or {}).copy()
    ...
    def test_no_llm_yields_error_message(self):
        ...
        error_msgs = [
            r for r in results if any(
                (getattr(m, 'content', '') if not isinstance(m, dict) else m.get('content', '')) and 'no LLM configured'
                in str(getattr(m, 'content', '') if not isinstance(m, dict) else m.get('content', '')) for m in r)
        ]
