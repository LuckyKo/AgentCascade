...
class TestBasicResolution:
    ...
    def test_resolves_single_placeholder(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args, 's1', 'read_file', agent_pool)
        assert err is None
    ...
    def test_resolves_string_placeholder(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args, 's1', 'any_tool', agent_pool)
        assert err is None
...
class TestScopeIsolation:
    ...
    def test_different_scopes_isolated(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args, 'scopeB', 't', agent_pool)
    ...
    def test_correct_scope_resolves(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders({'key': '{USE_CACHED_ENTRY_1}'}, 's1', 't', agent_pool)
        assert err is None
...
class TestMultipleEntries:
    ...
    def test_resolve_entry_1_and_2(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args, 's1', 't', agent_pool)
        assert err is None
...
class TestDeepCopy:
    ...
    def test_resolved_value_is_deep_copied(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders({'nested': '{USE_CACHED_ENTRY_1}'}, 's1', 't', agent_pool)
        assert err is None
        ...
        # Re-resolve
        resolved2, _ = resolve_prev_arg_placeholders({'nested': '{USE_CACHED_ENTRY_1}'}, 's1', 't', agent_pool)
...
class TestThreadSafety:
    ...
    def test_no_deadlock_with_lock_none(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders({'a': '{USE_CACHED_ENTRY_1}'}, 's1', 't', agent_pool, lock=None)
        assert err is None
    ...
    def test_lock_provided_works(self, agent_pool):
        ...
        l = threading.Lock()
        resolved, err = resolve_prev_arg_placeholders({'x': '{USE_CACHED_ENTRY_1}'}, 's1', 't', agent_pool, lock=l)
        assert err is None
    ...
    def test_concurrent_resolution_with_lock(self, agent_pool):
        ...
        def resolve_then_update(i):
            try:
                resolved, err = resolve_prev_arg_placeholders({'val': '{USE_CACHED_ENTRY_1}'},
                                                              's1',
                                                              't',
                                                              agent_pool,
                                                              lock=l)
    ...
    def test_no_cache_pool_attribute(self):
        ...
        tool_args = {'key': '{USE_CACHED_ENTRY_1}'}
        resolved, err = resolve_prev_arg_placeholders(tool_args, 's1', 't', bare_pool)
        assert err is None
...
class TestFullResolutionCycle:
    ...
    def test_resolve_use_cache_write_then_resolve_again(self, agent_pool):
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args_2, 'Maine', 'write_file', agent_pool)
        assert err is None
        ...
        resolved, err = resolve_prev_arg_placeholders(tool_args_4, 'Maine', 'write_file', agent_pool)
        assert err is None
