...
class ReIndentTester:
    ...
    def test_min_01_basic_spaces(self):
        ...
        print('\n── MIN Mode ──')
        p = self._write('min1.py',
                        '    def foo():\n        pass\n    bar()\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='min1.py',
        ...
        expected = '  def foo():\n      pass\n  bar()\n'
        self.ok('MIN-1 basic spaces', content == expected and 'OK:' in res, f"got [{content}]")
        if content != expected:
            self.bug('min', 'basic_spaces', '    def\n        pass\n    bar', expected, content)

    def test_min_02_tabs(self):
        """MIN-2: Re-indent with tabs."""
        p = self._write('min2.py',
                        '\t\tdef foo():\n\t\t\tpass\n\tbar()\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='min2.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=1,
                                    indent_type='tab',
    ...
    def test_min_03_mixed_levels(self):
        ...
        expected = '     a\n    b\n  c\nd\n'
        self.ok('MIN-3 mixed levels preserved', content == expected and 'OK:' in res, f"got [{content}]")

    def test_min_04_blank_lines(self):
        """MIN-4: Blank lines in the middle pass through unchanged."""
        p = self._write('min4.py', '    a\n\n    b\n  \n    c\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='min4.py',
                                    agent_name='test_agent',
                                    lines='1:5',
                                    indent=2,
                                    indent_type='space',
        ...
        expected = '  a\n\n  b\n\n  c\n'
        self.ok('MIN-4 blank lines preserved', content == expected and 'OK:' in res, f"got [{content}]")

    def test_min_05_single_line(self):
        """MIN-5: Single line block."""
        p = self._write('min5.py', '    only_line\nother\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='min5.py',
                                    agent_name='test_agent',
                                    lines='1:1',
                                    indent=2,
                                    indent_type='space',
        ...
        expected = '  only_line\nother\n'
        self.ok('MIN-5 single line block', content == expected and 'OK:' in res, f"got [{content}]")

    def test_min_06_all_blank(self):
        """MIN-6: All blank lines block."""
        p = self._write('min6.py', '\n\n\nother\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='min6.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=4,
                                    indent_type='space',
        ...
        expected = '\n\n\nother\n'
        self.ok('MIN-6 all blank block', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_shift_07_positive_spaces(self):
        ...
        res = self.op_mgr.re_indent(path='sh7.py',
                                    agent_name='test_agent',
                                    lines='1:2',
                                    indent=4,
                                    indent_type='space',
        ...
        expected = '    def foo():\n    bar()\n'
        self.ok('SHIFT-7 positive spaces', content == expected and 'OK:' in res, f"got [{content}]")

    def test_shift_08_positive_tabs(self):
        """SHIFT-8: Positive shift — add tabs."""
        p = self._write('sh8.py', 'def foo():\nbar()\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='sh8.py',
                                    agent_name='test_agent',
                                    lines='1:2',
                                    indent=3,
                                    indent_type='tab',
        ...
        expected = '\t\t\tdef foo():\n\t\t\tbar()\n'
        self.ok('SHIFT-8 positive tabs', content == expected and 'OK:' in res, f"got [{content}]")

    def test_shift_09_negative(self):
        """SHIFT-9: Negative shift — remove chars (type-agnostic)."""
        p = self._write('sh9.py',
                        '        deep()\n            deeper()\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='sh9.py',
                                    agent_name='test_agent',
                                    lines='1:2',
                                    indent=-4,
                                    indent_type='space',
    ...
    def test_shift_10_over_remove(self):
        ...
        expected = 'a\nb\n'
        self.ok('SHIFT-10 over-remove clamp', content == expected and 'OK:' in res, f"got [{content}]")

    def test_shift_11_zero_noop(self):
        """SHIFT-11: Zero indent — no-op."""
        p = self._write('sh11.py', '    a\n\t\tb\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='sh11.py',
                                    agent_name='test_agent',
                                    lines='1:2',
                                    indent=0,
                                    indent_type='space',
        ...
        expected = '    a\n\t\tb\n'
        self.ok('SHIFT-11 zero no-op', content == expected and 'OK:' in res, f"got [{content}]")

    def test_shift_12_mixed_ws(self):
        """SHIFT-12: Mixed whitespace types in input."""
        p = self._write('sh12.py', '\t  mixed()\n  \tabc()\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='sh12.py',
                                    agent_name='test_agent',
                                    lines='1:2',
                                    indent=-3,
                                    indent_type='space',
        ...
        expected = 'mixed()\nabc()\n'
        self.ok('SHIFT-12 mixed ws removal', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_flat_13_spaces(self):
        ...
        res = self.op_mgr.re_indent(path='fl13.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=2,
                                    indent_type='space',
        ...
        expected = '  a\n  b\n  c\n'
        self.ok('FLAT-13 flat spaces', content == expected and 'OK:' in res, f"got [{content}]")

    def test_flat_14_tabs(self):
        """FLAT-14: All lines flattened to same indent level with tabs."""
        p = self._write('fl14.py', '      a\n  b\n\t\tc\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='fl14.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=2,
                                    indent_type='tab',
        ...
        # All → 2 tabs prefix
        expected = '\t\ta\n\t\tb\n\t\tc\n'
        self.ok('FLAT-14 flat tabs', content == expected and 'OK:' in res, f"got [{content}]")

    def test_flat_15_blank_preserved(self):
        """FLAT-15: Blank lines preserved in flat mode."""
        p = self._write('fl15.py', '    a\n\n      b\n  c\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='fl15.py',
                                    agent_name='test_agent',
                                    lines='1:4',
                                    indent=3,
                                    indent_type='space',
        ...
        expected = '   a\n\n   b\n   c\n'
        self.ok('FLAT-15 blank preserved', content == expected and 'OK:' in res, f"got [{content}]")

    def test_flat_16_single_line(self):
        """FLAT-16: Single line block."""
        p = self._write('fl16.py', '      only\nother\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='fl16.py',
                                    agent_name='test_agent',
                                    lines='1:1',
                                    indent=4,
                                    indent_type='space',
        ...
        expected = '    only\nother\n'
        self.ok('FLAT-16 single line', content == expected and 'OK:' in res, f"got [{content}]")

    def test_flat_17_mixed_uniform(self):
        """FLAT-17: Mixed indentation levels all become uniform."""
        p = self._write('fl17.py', '\t\ta\n\tb\n  c\n d\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='fl17.py',
                                    agent_name='test_agent',
                                    lines='1:4',
                                    indent=0,
                                    indent_type='space',
        ...
        expected = 'a\nb\nc\nd\n'
        self.ok('FLAT-17 mixed→uniform zero', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_convert_18_spaces_to_tabs(self):
        ...
        res = self.op_mgr.re_indent(path='cv18.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=4,
                                    indent_type='tab',
    ...
    def test_convert_19_tabs_to_spaces(self):
        ...
        expected = '            a\n        b\n    c\n'
        self.ok('CONVERT-19 tabs→spaces', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_convert_20_mixed_ws(self):
        ...
        res = self.op_mgr.re_indent(path='cv20.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=4,
                                    indent_type='space',
        ...
        expected = '          a\n           b\n    c\n'
        self.ok('CONVERT-20 mixed ws', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_convert_21_hierarchy(self):
        ...
        p = self._write(
            'cv21.py',
            '    if True:\n        pass\n    else:\n        pass\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='cv21.py',
                                    agent_name='test_agent',
                                    lines='1:4',
                                    indent=4,
                                    indent_type='tab',
        ...
        expected = '\tif True:\n\t\tpass\n\telse:\n\t\tpass\n'
        self.ok('CONVERT-21 hierarchy preserved', content == expected and 'OK:' in res, f"got [{content}]")

    def test_convert_22_blank_preserved(self):
        """CONVERT-22: Blank lines preserved in convert mode."""
        p = self._write('cv22.py', '    a\n\n        b\n  c\n')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='cv22.py',
                                    agent_name='test_agent',
                                    lines='1:4',
                                    indent=4,
                                    indent_type='space',
        ...
        expected = '      a\n\n          b\n    c\n'
        self.ok('CONVERT-22 blank preserved', content == expected and 'OK:' in res, f"got [{content}]")
    ...
    def test_edge_23_empty_file(self):
        ...
        print('\n── Edge Cases ──')
        p = self._write('edge23.py', '')  # noqa: F841  (temp file path kept as statement)
        res = self.op_mgr.re_indent(path='edge23.py',
                                    agent_name='test_agent',
                                    lines='1:5',
                                    indent=4,
                                    indent_type='space',
    ...
    def test_edge_24_oob_ranges(self):
        ...
        # start too high
        res1 = self.op_mgr.re_indent(path='edge24.py',
                                     agent_name='test_agent',
                                     lines='5:10',
                                     indent=2,
                                     indent_type='space',
        ...
        # end too high
        res2 = self.op_mgr.re_indent(path='edge24.py',
                                     agent_name='test_agent',
                                     lines='1:10',
                                     indent=2,
                                     indent_type='space',
    ...
    def test_edge_25_invalid_mode(self):
        ...
        res = self.op_mgr.re_indent(path='edge25.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=2,
                                    indent_type='space',
    ...
    def test_edge_26_negative_indent(self):
        ...
        res1 = self.op_mgr.re_indent(path='edge26.py',
                                     agent_name='test_agent',
                                     lines='1:3',
                                     indent=-2,
                                     indent_type='space',
        ...
        res2 = self.op_mgr.re_indent(path='edge26b.py',
                                     agent_name='test_agent',
                                     lines='1:3',
                                     indent=-2,
                                     indent_type='space',
    ...
    def test_edge_27_crlf(self):
        ...
        res = self.op_mgr.re_indent(path='edge27.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=2,
                                    indent_type='space',
        ...
        expected = '    a\n        b\n  c\n'
        self.ok('EDGE-27 CRLF preserved', content == expected and 'OK:' in res, f"got [{repr(content)}]")
    ...
    def test_edge_28_mixed_endings(self):
        ...
        res = self.op_mgr.re_indent(path='edge28.py',
                                    agent_name='test_agent',
                                    lines='1:3',
                                    indent=0,
                                    indent_type='space',
        ...
        expected = '  a\n      b\nc\n'
        self.ok('EDGE-28 mixed endings', content == expected and 'OK:' in res, f"got [{repr(content)}]")
