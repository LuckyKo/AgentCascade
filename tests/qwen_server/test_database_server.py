...
from agent_server.utils import read_meta_data_by_condition


def test_database_server():
    server_config_path = Path(__file__).resolve().parent.parent.parent / 'agent_server/server_config.json'
    ...
    # cache
    from agent_server.database_server import cache_page, update_pop_url

    data = {
        'url':
            'https://github.com/QwenLM/AgentCascade',
        'content':
            '<p>AgentCascade is a framework for developing LLM applications based on the instruction following, tool usage, planning, and memory capabilities of Qwen. </p>'
    }
    cache_page(**data)
