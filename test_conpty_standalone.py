"""
Standalone ConPTY test — copies diag7_final pattern exactly.
Uses its own struct definitions (not from async_shell) to isolate issues.
"""
...
# EXACT argtypes from MS docs: lpStartupInfo is POINTER(STARTUPINFOW)
...
def main():
    print("=" * 60, flush=True)
    print("Standalone ConPTY Test (exact diag7_final copy)", flush=True)
    print("=" * 60, flush=True)
    ...
    command = ctypes.c_wchar_p('cmd.exe /c "echo Hello from ConPTY && echo Second line"')
    ...
    success = kernel32.CreateProcessW(
        ...
        ctypes.byref(si.StartupInfo),  # lpStartupInfo — pointer to STARTUPINFOW member per MS docs!
