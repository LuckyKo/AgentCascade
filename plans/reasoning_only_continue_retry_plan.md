...
- **Turn budget** (UPDATED 2026-08-25, commit 3472d0a): the old `turns_available` reset on auto-continue was hard-removed — every auto-continue (soft or full) now consumes exactly one real turn, so `max_turns` is a true hard budget. Regression tests: `tests/test_auto_continue_turn_budget.py`.
