...
## Refinement cleanup (post-commit 7743cd6)

Two pure style refactors from refinement review — **no behavior change** (constant relocation + naming only).

1. **`TAIL_THRESHOLD` moved to module level** (`state_builder.py`). Was defined inline inside `_serialize_instance`; now a module-level constant (after the imports, ~line 25) with a clear comment, consistent with how other constants are defined. The reference in `_serialize_instance` now resolves to the module constant. Value unchanged (50).

2. **Named the non-streaming periodic interval** (`streaming.py`). The hardcoded `0.1` in `should_broadcast` is now `PERIODIC_BROADCAST_INTERVAL = 0.1`, a module-level constant placed next to `MIN_STREAM_BROADCAST_INTERVAL` (which was also moved from inline to module level for consistency). Value and comparison operator unchanged — behavior identical.

Verification:
- Syntax check: both files valid.
- Tests: `python -m pytest tests/test_streaming_buffering_fixes.py tests/test_state_builder.py tests/test_api_endpoints.py -v` → **51 passed, 0 failed** (identical to pre-refactor).
