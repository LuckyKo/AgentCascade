...
This fork adds a production-oriented multi-agent framework, a custom web UI, and several robustness improvements over the upstream `QwenLM/AgentCascade` main branch. **899 insertions across 24 files.**
...
AgentCascade is a framework for developing LLM applications based on the instruction following, tool usage, planning, and
...
Now AgentCascade plays as the backend of [Qwen Chat](https://chat.qwen.ai/).
...
For the QwQ and Qwen3 model, it is recommended to **do not** add the `--enable-auto-tool-choice` and `--tool-call-parser hermes` parameters, as AgentCascade will parse the tool outputs from vLLM on its own.
...
AgentCascade offers atomic components, such as LLMs (which inherit from `class BaseChatModel` and come with [function calling](https://github.com/QwenLM/AgentCascade/blob/main/examples/function_calling.py)) and Tools (which inherit
...
```
Now you can chat with the Agent in the web UI. Please refer to the [examples](https://github.com/QwenLM/AgentCascade/blob/main/examples) directory for more usage examples.
...
Yes. The LLM classes provide [function calling](https://github.com/QwenLM/AgentCascade/blob/main/examples/function_calling.py). Additionally, some Agent classes also are built upon the function calling capability, e.g., FnCallAgent and ReActChat.
...
We have released [a fast RAG solution](https://github.com/QwenLM/AgentCascade/blob/main/examples/assistant_rag.py), as well as [an expensive but competitive agent](https://github.com/QwenLM/AgentCascade/blob/main/examples/parallel_doc_qa.py), for doing question-answering over super-long documents. They have managed to outperform native long-context models on two challenging benchmarks while being more efficient, and perform perfectly in the single-needle "needle-in-the-haystack" pressure test involving 1M-token contexts. See the [blog](https://qwenlm.github.io/blog/agent-cascade-2405/) for technical details.
...
BrowserQwen is a browser assistant built upon AgentCascade. Please refer to its [documentation](https://github.com/QwenLM/AgentCascade/blob/main/browser_agent.md) for details.
